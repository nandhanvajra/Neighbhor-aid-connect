...
router.post("/group", adminAuth, async (req, res) => {
  ...
  try {
    ...
    const participants = [...participantSet].map(
...
router.post("/create", auth, async (req, res) => {
  ...
  if (!participant1 || !participant2) {
    return res.status(400).json({
      message: "Both participants are required",
    });
  }
  const uid = String(req.user.userId);
  if (uid !== String(participant1) && uid !== String(participant2)) {
    return res.status(403).json({
      message: "You can only create chats with yourself and another user",
    });
  }
  try {
...
router.post("/:chatId/messages", auth, async (req, res) => {
  ...
  try {
    ...
    const isGroupChat =
      chat.chatType === "group" || chat.participants.length > 2;
    if (!isGroupChat) {
      ...
      if (otherParticipant) {
        ...
        if (!activeRequest) {
          ...
          if (completedRequest) {
            return res.status(403).json({
              message:
                "Cannot send messages. The service request has been completed.",
            });
          }
        }
      }
