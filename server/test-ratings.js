...
async function createTestRatings() {
  try {
    ...
    if (existingRatings > 0) {
      console.log(
        `Found ${existingRatings} existing ratings. Skipping test creation.`,
      );
      return;
    }
    const testRequests = [];
    ...
    const ratingData = [
      ...
      {
        stars: 4,
        review: "Good work, completed the task efficiently.",
        qualityOfWork: 4,
        communication: 4,
        professionalism: 4,
      },
      {
        stars: 5,
        review: "Amazing helper! Would definitely recommend.",
        qualityOfWork: 5,
        communication: 5,
        professionalism: 5,
      },
    ...
    if (helper) {
      await helper.updateRatingStats(5);
