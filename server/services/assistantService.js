...
async function getTopWorkersByRating(intent) {
  ...
  if (intent.workerType) {
    query.job = {
      $regex: `^${escapeRegex(intent.workerType)}$`,
      $options: "i",
    };
  }
...
async function getWorkerFeedbackSummary(intent) {
  ...
  const reviews = await Rating.find({
    ...
    .limit(5)
    .select(
      "stars review qualityOfWork communication professionalism category createdAt",
    )
    .lean();
  return {
    worker: {
...
async function getWorkerLookup(intent) {
  ...
  return {
    ...
    averageRating: worker?.rating?.average || 0,
    totalRatings: worker?.rating?.totalRatings || 0,
    ratingBreakdown: worker?.rating?.ratingBreakdown || {
      1: 0,
      2: 0,
      3: 0,
      4: 0,
...
async function handleResidentAssistantQuery(message) {
  ...
  if (intent.intent === "unsupported") {
    return {
      ...
      data: results,
      reply:
        "I can help with worker rankings, activity, and feedback insights. Try asking about top workers, completed jobs, or reviews.",
    };
  }
