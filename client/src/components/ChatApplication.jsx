...
export default function ChatApplication() {
  ...
  useEffect(() => {
    if (!chatid || !user) {
      ...
      setLoading(false);
      setError(
        "No chat ID or user found. Please navigate to a valid chat room.",
      );
      return;
    }
    console.log(
      "Setting up chat for chatid:",
      chatid,
      "with user:",
      user._id,
      "group:",
      ...
      try {
        ...
        if (isGroupChat) {
          const chatRes = await fetch(
            `${config.apiBaseUrl}/api/chats/${chatid}`,
            {
              headers: {
          ...
          if (!chatRes.ok) {
            ...
            throw new Error(
              errBody.message || `Failed to load group chat: ${chatRes.status}`,
            );
          }
          ...
          const chatResponse = await fetch(
            `${config.apiBaseUrl}/api/chats/create`,
            {
              method: "POST",
              headers: {
          ...
          if (!chatResponse.ok) {
            throw new Error(
              `Failed to create/get chat: ${chatResponse.status}`,
            );
          }
          ...
          const checkRequestStatus = async () => {
            try {
              const requestsResponse = await fetch(
                `${config.apiBaseUrl}/api/requests/all`,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
              ...
              if (requestsResponse.ok) {
                ...
                if (
                  ...
                  const activeRequest = requestsData.requests.find(
                    (req) =>
                      (req.status === "pending" ||
                        req.status === "in-progress") &&
                      ((req.userId === user._id &&
                        req.completedBy === chatid) ||
                        (req.userId === chatid &&
                          req.completedBy === user._id)),
                  );
                  if (activeRequest) {
                    setIsRequestCompleted(false);
                  } else {
                    const completedRequest = requestsData.requests.find(
                      (req) =>
        ...
        const messagesResponse = await fetch(
          `${config.apiBaseUrl}/api/chats/${resolvedChatId}/messages`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const toggleSpeechToText = useCallback(() => {
    ...
    if (typeof window !== "undefined" && window.isSecureContext === false) {
      setError(
        "Speech-to-text requires HTTPS or localhost. Please use a secure connection.",
      );
      return;
    }
    ...
    rec.onerror = (ev) => {
      if (ev.error === "not-allowed") {
        setError(
          "Microphone access denied. Allow the mic and ensure you are on HTTPS or localhost.",
        );
        ...
        <div className="mx-4 mt-2 bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          ...
          {user?.isAdmin ? (
            <p className="px-4 py-2 text-xs text-gray-600 border-t border-gray-100 bg-gray-50">
              To add or remove residents, use Dashboard → Chats → Manage on this
              group.
            </p>
          ) : (
            <p className="px-4 py-2 text-xs text-gray-500 border-t border-gray-100 bg-amber-50/50">
              Member list is read-only. Only admins can change members from the
              dashboard.
            </p>
          )}
        </div>
            ...
            {loading ? (
              ...
              messages.map((msg, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`px-4 py-2 rounded-lg max-w-xs md:max-w-md lg:max-w-lg break-words ${isCurrentUser ? "bg-orange-500 text-white rounded-br-none" : "bg-gray-200 text-gray-800 rounded-bl-none"} ${msg.pending ? "opacity-60" : ""}`}
                    >
                      {!isCurrentUser && (
                      ...
                      <div className="flex items-start gap-2 justify-between">
                        ...
                        {ttsSupported && msg.text && (
                          <button
                            ...
                            className={`shrink-0 p-1 rounded-md border ${speakingMsgId === msg._id ? (isCurrentUser ? "border-white/80 bg-white/20 text-white" : "border-orange-400 bg-orange-100 text-orange-800") : isCurrentUser ? "border-white/40 text-white/90 hover:bg-white/10" : "border-gray-300 text-gray-600 hover:bg-gray-100"}`}
                            aria-label={
                              speakingMsgId === msg._id
                                ? "Stop reading"
                                : "Read message aloud"
          ...
          <div className="p-3 border-t">
            {!isGroupChat && isRequestCompleted && (
              <div className="mb-2 p-2 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800 text-center">
                This service request has been completed. You can view the chat
            ...
            <form onSubmit={sendMessage} className="flex items-stretch gap-0">
              {sttSupported && (
                <button
                  ...
                  disabled={
                    loading || !user || (!isGroupChat && isRequestCompleted)
                  }
                  title={
                    listening ? "Stop dictation" : "Speak to type (microphone)"
                  }
                  className={`shrink-0 px-3 py-2 border border-r-0 border-gray-300 rounded-l-full transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${listening ? "bg-red-500 text-white border-red-500 animate-pulse" : "bg-white text-gray-700 border-gray-300 hover:border-orange-400 hover:bg-orange-50"}`}
                  aria-pressed={listening}
                  aria-label={
                    listening ? "Stop dictation" : "Start speech to text"
                  }
                >
                  <svg
                    className="h-5 w-5"
              ...
              <input
                ...
                className={`flex-1 min-w-0 border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed ${sttSupported ? "rounded-none border-l-0" : "rounded-l-full"}`}
                placeholder={
                  !isGroupChat && isRequestCompleted
            ...
            {(sttSupported || ttsSupported) && (
              <p className="text-xs text-gray-500 mt-2 text-center">
                {sttSupported && (
                  <span>
                    <strong>Speech to text:</strong> mic button (Chrome/Edge
                    recommended).{" "}
                  </span>
                )}
                {ttsSupported && (
                  <span>
                    <strong>Text to speech:</strong> speaker icon on each
                    message.
                  </span>
                )}
              </p>
