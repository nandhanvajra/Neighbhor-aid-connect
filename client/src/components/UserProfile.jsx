...
export default function UserProfile() {
  ...
  const fetchUserStats = async (userId) => {
    try {
      ...
      const solved = data.requests.filter(
        (r) => r.userId === userId && r.status === "completed",
      ).length;
      const helped = data.requests.filter(
        (r) =>
  ...
  const addSkill = () => {
    if (
      skillInput.trim() &&
      formData.skills.length < config.profile.maxSkillsCount
    ) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${config.apiBaseUrl}/api/auth/user/profile`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...
            {ratingStats && ratingStats.totalRatings > 0 && (
              <div className="bg-orange-50 rounded-lg p-4 flex-1 text-center">
                <div className="flex items-center justify-center mb-1">
                  <div className="text-2xl font-bold text-orange-600 mr-2">
                  ...
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        size={16}
                        className={`${star <= Math.round(ratingStats.averageRating) ? "text-yellow-500 fill-current" : "text-gray-300"}`}
                      />
                    ))}
                  </div>
                </div>
                <div className="text-sm text-gray-600 mb-2">
                  Average Rating ({ratingStats.totalRatings} ratings)
                </div>
                <button
                  onClick={() =>
                    (window.location.href = `/ratings/${user._id || user.id}`)
                  }
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      className={`pl-10 w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 ${!editing ? "bg-gray-50 cursor-not-allowed" : ""}`}
                      value={formData.name}
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    className={`w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 ${!editing ? "bg-gray-50 cursor-not-allowed" : ""}`}
                    value={formData.bio}
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {formData.bio.length}/{config.profile.maxBioLength}{" "}
                    characters
                  </p>
                </div>
            ...
            {isWorker && (
              <div className="mt-8 pt-8 border-t">
                ...
                <div
                  className={`w-full border border-gray-300 rounded px-2 py-2 ${!editing ? "bg-gray-50 cursor-not-allowed" : ""}`}
                  style={{
                    minHeight: "48px",
                  }}
                >
                  {editing ? (
                    <div className="flex flex-wrap gap-2">
                      {config.serviceCategories.map((cat) => (
                        <button
                          key={cat.value}
                          type="button"
                          className={`px-3 py-1 rounded-full border text-sm flex items-center gap-1 transition-colors ${requestPreferences.includes(cat.value) ? "bg-orange-500 text-white border-orange-500" : "bg-white text-gray-700 border-gray-300 hover:bg-orange-50"}`}
                          onClick={() => {
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {requestPreferences.map((val) => {
                        const cat = config.serviceCategories.find(
                          (c) => c.value === val,
                        );
                        return (
                          <span
