...
export default function UserProfilePage({ editMode }) {
  ...
  console.log(
    "UserProfilePage rendered with userId:",
    userId,
    "editMode:",
    editMode,
  );
  useEffect(() => {
    console.log(
      "UserProfilePage useEffect triggered with userId:",
      userId,
      "editMode:",
      editMode,
    );
  ...
  const fetchUserRatings = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/api/users/${userId}/ratings`,
      );
      if (response.ok) {
        const data = await response.json();
                ...
                {(user.rating > 0 || averageRating > 0) && (
                  <div className="flex items-center justify-center mt-2">
                    {Array.from({
                      ...
                      <svg
                        key={i}
                        className={`w-5 h-5 ${i < Math.round(user.rating || averageRating) ? "fill-current text-yellow-400" : "text-gray-300"}`}
                        viewBox="0 0 20 20"
                      >
                    ...
                    <span className="ml-2 text-yellow-200 font-semibold">
                      {(user.rating || averageRating).toFixed(2)} / 5
                    </span>
                    <span className="ml-2 text-orange-100 text-xs">
                      {user.totalRatings || totalRatings} rating
                      {(user.totalRatings || totalRatings) !== 1 ? "s" : ""}
                    </span>
                  </div>
                )}
              </div>
